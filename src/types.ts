 ...
 * 生の数値の代わりに IchigoJamButton.Left のように使える。英字・数字は
 * ASCII コード (例: 'A' = 65) をそのまま渡す。
 */
export const IchigoJamButton = {
  Left: 28,
  Right: 29,
  Up: 30,
  Down: 31,
...
export type IchigoJamButton = (typeof IchigoJamButton)[keyof typeof IchigoJamButton];

/**
 * peek(addr) で読めるメモリ空間の先頭アドレス一覧 (ram.h の仮想アドレス
 * マップに対応)。生の 16 進数の代わりに IchigoJamMemory.Vram のように使える。
 *
...
export type IchigoJamMemory = (typeof IchigoJamMemory)[keyof typeof IchigoJamMemory];
 ...
 */
export interface IchigoJamError {
...
/**
 * 命令ハンドル: ref 経由で動作中の IchigoJam を外部制御する。
 * すべて wasm ランナーの公開メソッドへ委譲する。
 */
export interface IchigoJamHandle {
  ...
  reset(): void;
  /** INKEY()/BTN() 用の物理キー押下 (IchigoJamButton または ASCII コード)。 */
  ...
  /**
   * メモリ (PEEK 相当) を読む。有効アドレス空間は IchigoJamMemory 参照
...
export interface IchigoJamProps {
   ...
   * IchigoJam コンポーネントの値がページ全体に適用される。以降のインスタンスで

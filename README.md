...
`peek` には生の 16 進数の代わりに `IchigoJamMemory` (`RomFont`/`Pcg`/`Var`/`Vram`/`List`/`Keybuf`/`Linebuf`/`I2cbuf`) を渡せる。有効アドレス空間は `#000`-`#117F` で、範囲外・不正な値でも `peek` は例外を投げず `0` を返す (実機の PEEK と同じ挙動)。返ってきた `0` が実データか範囲外による既定値かを区別したいときは `isValidAddress(addr)` で事前に判定する。

```ts
import { IchigoJamMemory, isValidAddress } from "ichigojam-react";

isValidAddress(IchigoJamMemory.Vram); // true
isValidAddress(0x2000); // false (アドレス空間外)
```
